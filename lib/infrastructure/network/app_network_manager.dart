...
class AppNetworkManager implements IAppNetworkManager {
  ...
  static final AppNetworkManager _instance = AppNetworkManager._();

  AppNetworkManager._() {
    _manager = NetworkManager<ResponseModel>();
    init();
  }
  ...
  Future<void> init() async {
    await _manager.initialize(
      NetworkConfiguration(
        OpenAIConfig.instance.apiBaseUrlWithVersion,
        minimumValidateStatus: 500,
      ),
      responseModel: ResponseModel(),
      entityKey: 'data',
    );

    _manager.addHeader({'OpenAI-Beta': 'assistants=v1'});
  ...
  @override
  Future<void> changeLangHeader(String langCode) async {
    await waitForManagerInitialization;
  ...
  @override
  Future<void> addAuthorizationHeader(String token) async {
    await waitForManagerInitialization;
